...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        conv1_weights = tf.get_variable("weight",[5,5,3,32],initializer=tf.truncated_normal_initializer(stddev=0.1)) #[width, height, channels, kernel_nums]
        conv1_biases = tf.get_variable("bias", [32], initializer=tf.constant_initializer(0.0))
        conv1 = tf.nn.conv2d(input_tensor, conv1_weights, strides=[1, 1, 1, 1], padding='SAME')#[batch_size, image_rows, image_cols, number_of_colors]，"VALID" = without padding，"SAME" = with zero padding.
